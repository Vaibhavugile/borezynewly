    ...
    const applyFilter = () => {
      ...
      filtered = filtered.filter(lead => {
        ...
        if (searchField === 'nextFollowup') {
          // Ensure lead.nextFollowup is a valid date string before attempting to convert
          const followupDateStr = lead.nextFollowup ? new Date(lead.nextFollowup).toDateString() : '';
          const queryDateStr = new Date(lowerCaseQuery).toDateString();
          return queryDateStr === followupDateStr;
        } else {
          // Ensure lead[searchField] exists before calling toLowerCase()
